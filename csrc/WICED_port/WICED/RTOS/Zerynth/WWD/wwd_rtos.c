...
wwd_result_t host_rtos_create_thread( host_thread_type_t* thread, void(*entry_function)( uint32_t ), const char* name, /*@null@*/ void* stack, uint32_t stack_size, uint32_t priority )
{
    ...
    *thread = vosThCreate(stack_size,priority,entry_function,NULL,NULL);
    vosThResume(*thread);
...
wwd_result_t host_rtos_create_thread_with_arg( host_thread_type_t* thread, void(*entry_function)( uint32_t ), const char* name, /*@null@*/ void* stack, uint32_t stack_size, uint32_t priority, uint32_t arg )
{
    ...
    *thread = vosThCreate(stack_size,priority,entry_function,(void*)arg,NULL);
    vosThResume(*thread);
...
{
    *semaphore = vosSemCreate(0);
    zprintf("Creating semaphore %x\n",*semaphore); 
    return WWD_SUCCESS;
}
...
wwd_result_t host_rtos_get_semaphore( host_semaphore_type_t* semaphore, uint32_t timeout_ms, wiced_bool_t will_set_in_isr )
{
   ...
   // zprintf("getting semaphore %x %i\n",*semaphore,timeout_ms);
    int ret = vosSemWaitTimeout(*semaphore,(timeout_ms==NEVER_TIMEOUT) ? VTIME_INFINITE:(TIME_U(timeout_ms,MILLIS)));
    if (ret==VRES_TIMEOUT){
        zprintf("semaphore %x timeout after %i\n",*semaphore,timeout_ms);
        return WWD_TIMEOUT;
    }
    //zprintf("semaphore %x released\n",*semaphore); 
    return WWD_SUCCESS;
}
...
wwd_result_t host_rtos_set_semaphore( host_semaphore_type_t* semaphore, wiced_bool_t called_from_ISR )
{
    ...
    if (called_from_ISR==WICED_TRUE){
        vosSysLockIsr();
        vosSemSignalIsr(*semaphore);
        vosSysUnlockIsr();
    } else {
        zprintf("set semaphore %x %i\n",*semaphore,called_from_ISR);
        vosSemSignal(*semaphore);
    }
...
wwd_result_t host_rtos_deinit_semaphore( host_semaphore_type_t* semaphore )
{
    ...
    zprintf("destroy semaphore %x\n",*semaphore);
    vosSemDestroy(*semaphore);
...
{
    return (wwd_time_t) _systime_millis;
}
...
wwd_result_t host_rtos_delay_milliseconds( uint32_t num_ms )
{
    ...
    //WPRINT_MACRO(("wait for %i from %i\n",num_ms,start));
    //num_ms++;
    vosThSleep(TIME_U(num_ms,MILLIS));
