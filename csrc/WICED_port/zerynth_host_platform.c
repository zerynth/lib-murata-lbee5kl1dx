...
wwd_result_t host_platform_init( void )
{
    ...
    zprintf("+host_platform_init 2\n");
    vhalPinSetMode(WWD_RESET_PIN,PINMODE_OUTPUT_PUSHPULL);
    host_platform_reset_wifi( WICED_TRUE );  /* Start wifi chip in reset */
#endif

#if defined ( WICED_USE_WIFI_POWER_PIN )
    zprintf("+host_platform_init 3 %i\n",WWD_POWER_PIN);
    vhalPinSetMode(WWD_POWER_PIN,PINMODE_OUTPUT_PUSHPULL);
    host_platform_power_wifi( WICED_FALSE ); /* Start wifi chip with regulators off */
    zprintf("+host_platform_init 4\n");
...
{
#if defined ( WICED_USE_WIFI_RESET_PIN )
    ( reset_asserted == WICED_TRUE ) ? vhalPinWrite(WWD_RESET_PIN,0) : vhalPinWrite(WWD_RESET_PIN,1);
#else
